...
mint ans;
int f[26];
int sum[1 << 24], mn[MAX + 5];
mint dp[1005][MAX + 5];

void solve() {
    f[1] = f[2] = 1;
    FOR (i, 3, 25) f[i] = f[i - 1] + f[i - 2];
    ...
    fill(mn, mn + lim + 1, 114514);
    FOR (i, 0, 23) sum[1 << i] = f[i + 1];
    FOR (mask, 1, (1 << 24) - 1) {
        sum[mask] = sum[mask & -mask] + sum[mask ^ (mask & -mask)];
        if (sum[mask] <= lim) mn[sum[mask]] = min(mn[sum[mask]], __builtin_popcount(mask));
    }

    dp[0][0] = 1;
    FOR (i, 1, x) {
        mx = min(mx + f[i] * n, lim);
        FOR (j, 0, n) for (int k = 0; k <= mx; k++) {
            if (j > 0 && k >= f[i]) dp[j][k] += dp[j - 1][k - f[i]];
        }
    }
    FOR (i, 1, mx) if (mn[i] == m) ans += dp[n][i];
    cout << ans.val() << '\n';
}

bool Y;
int main() {
    debug((&Y - &X) / 1048576.00);
    Waimai;
    solve();
}