...
signed main() {
    ...
    REP1(i,n) {
        REP(j,30) x[0][j][i]=(a[i]>>j)&1,x[1][j][i]=x[0][j][i]^1,px[0][j][i]=px[0][j][i-1]^x[0][j][i],px[1][j][i]=px[1][j][i-1]^x[1][j][i],p[0][j][i]=p[0][j][i-1]+px[0][j][i],p[1][j][i]=p[1][j][i-1]+(px[0][j][i]^1);
    }
    int an=0;
    REP1(i,n) {
        REP(j,30) {
