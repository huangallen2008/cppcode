...
signed main() {
    ...
    Vpii c;
    REP(i,n) {
        while(SZ(c)>=2&&cros(c.back()-c[SZ(c)-2],a[i]-c.back())<0) c.pop_back();
        c.pb(a[i]);
    }
    RREP(i,n-1) {
        while(SZ(c)>=2&&cros(c.back()-c[SZ(c)-2],a[i]-c.back())<0) c.pop_back();
    ...
    c.pop_back();
    Vpii c2=c;
    for(auto x:c) c2.pb(x);
    c2.pb(c[0]);
    auto len2=[&](pii a) { return SQ(a.f)+SQ(a.s); };
    int it=0,an=0;
    REP(i,SZ(c)) {
        while(len2(c2[it+1]-c2[i])>=len2(c2[it]-c2[i])) it++;
        chmax(an,len2(c2[it]-c2[i]));
    }
    ld ans=sqrtl(an);
